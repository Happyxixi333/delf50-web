...
function readingBody(p,c,day,slot){const pe=person(day,slot),name=pe[0],pro=pe[1],where=c[1],goal=c[2],issue=c[3],alt=c[4],fact=c[5],result=c[6],cityName=city(day,slot),when=dayName(day,slot),at=time(day,slot),g=grammarCue(p,name,pro,c),style=slot%4;let s='';
 ...
 return s}
function makeReading(old,p,day,slot,generation){const c=pickCase(p,day,slot,generation),pe=person(day,slot+(Number(generation)===181?1:0)),name=pe[0],pro=pe[1],cityName=city(day,slot+(Number(generation)===181?1:0)),text=readingBody(p,c,day,slot),issue=cap(c[3])+'.',solution=cap(c[4])+'.',detail=cap(c[5])+'.',result=cap(c[6])+'.',style=slot%4;
 const q1=[
  ['Pourquoi '+name+' '+(pro==='elle'?'écrit-elle':'écrit-il')+' ce message ?','Parce qu’une contrainte l’oblige à vérifier une solution avant de confirmer.','Pour obtenir une réduction qui n’est jamais évoquée.','Pour annoncer qu’il ou elle renonce définitivement à son projet.'],
  ...
  ['Pourquoi '+name+' '+(pro==='elle'?'raconte-t-elle':'raconte-t-il')+' cette expérience ?','Pour expliquer comment une difficulté a conduit à modifier une décision.','Pour décrire un lieu sans raconter ce qui s’est passé.','Pour montrer qu’aucune vérification n’était nécessaire.'],
